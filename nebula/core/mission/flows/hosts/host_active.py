...
class SetActiveTask(task.NebulaTask):
    default_provides = 'host'

    def __init__(self):
        super(SetActiveTask, self).__init__(addons=[ACTION])
    ...
    def revert(self, job_id, hostname, *args, **kwargs):
        ...
        if self.is_current_task_ok(rv):
            ...
            self.update_job_state_desc(job_id, _(u"恢复主机失败"))
...
class UpdateTask(task.NebulaTask):
    ...
    def execute(self, job_id, hostname, resource_id, **kwargs):
        ...
        self.update_job_state_desc(job_id, _(u"恢复主机成功"))
...
def get_flow():
    ...
    flow.add(SetActiveTask(),
             UpdateTask())
    return flow


class SetActiveBuilder(Builder):

    def __init__(self, context, resource_args=(), resource_kwargs=None, **kwargs):
        super(SetActiveBuilder, self).__init__(
