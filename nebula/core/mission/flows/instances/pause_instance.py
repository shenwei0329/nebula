...
class PauseOSInstanceTask(task.NebulaTask):
    def __init__(self):
        super(PauseOSInstanceTask, self).__init__(addons=[ACTION])

    def execute(self, job_id, instance_uuid, **kwargs):
        self.update_job_state_desc(job_id, _(u"开始暂停虚拟机"))
        nova.get_client().servers.pause(instance_uuid)
        ...
        def checker():
            ...
            elif instance_ref['vm_state'] == vm_states.PAUSED:
                # 状态为 active, 暂停完成
                return True
            return False
        ...
        # 等待完成或错误(包括超时)
        monitor.wait()
        self.update_job_state_desc(job_id, _(u"暂停虚拟机成功"))

    def revert(self, job_id, *args, **kwargs):
        self.log_current_task_failures(*args, **kwargs)
        self.update_job_state_desc(job_id, _(u"暂停虚拟机失败"))


def get_flow():
    flow = linear_flow.Flow(ACTION)
    flow.add(PauseOSInstanceTask())
    return flow


class InstancePauseBuilder(InstanceURDBuilder):
    def __init__(self, context, resource_kwargs=None, store=None):
        super(InstancePauseBuilder, self).__init__(
            context,
