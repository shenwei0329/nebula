...
class StartOSInstanceTask(task.NebulaTask):
    def __init__(self):
        super(StartOSInstanceTask, self).__init__(addons=[ACTION])

    def execute(self, job_id, instance_uuid, **kwargs):
        self.update_job_state_desc(job_id, _(u"开始启动虚拟机"))
        nova.get_client().servers.start(instance_uuid)
        ...
        # 等待完成或错误(包括超时)
        monitor.wait()
        self.update_job_state_desc(job_id, _(u"启动虚拟机成功"))

    def revert(self, job_id, *args, **kwargs):
        self.log_current_task_failures(*args, **kwargs)
        self.update_job_state_desc(job_id, _(u"启动虚拟机失败"))


def get_flow():
    flow = linear_flow.Flow(ACTION)
    flow.add(StartOSInstanceTask())
    return flow


class InstanceStartBuilder(InstanceURDBuilder):
    def __init__(self, context, resource_kwargs=None, store=None):
        super(InstanceStartBuilder, self).__init__(
            context,
