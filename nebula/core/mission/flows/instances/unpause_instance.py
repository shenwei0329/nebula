...
ACTION = 'instance:unpause'


class UnpauseOSInstanceTask(task.NebulaTask):
    def __init__(self):
        super(UnpauseOSInstanceTask, self).__init__(addons=[ACTION])

    def execute(self, job_id, instance_uuid, **kwargs):
        self.update_job_state_desc(job_id, _(u"开始恢复暂停的虚拟机"))
        nova.get_client().servers.unpause(instance_uuid)
        ...
        # 等待完成或错误(包括超时)
        monitor.wait()
        self.update_job_state_desc(job_id, _(u"恢复暂停的虚拟机成功"))

    def revert(self, job_id, *args, **kwargs):
        self.log_current_task_failures(*args, **kwargs)
        self.update_job_state_desc(job_id, _(u"恢复暂停的虚拟机失败"))


def get_flow():
    flow = linear_flow.Flow(ACTION)
    flow.add(UnpauseOSInstanceTask())
    return flow


class InstanceUnpauseBuilder(InstanceURDBuilder):
    def __init__(self, context, resource_kwargs=None, store=None):
        super(InstanceUnpauseBuilder, self).__init__(
            context,
