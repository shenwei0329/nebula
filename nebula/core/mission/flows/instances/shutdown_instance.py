...
class ShutdownOSInstanceTask(task.NebulaTask):
    def __init__(self):
        super(ShutdownOSInstanceTask, self).__init__(addons=[ACTION])

    def execute(self, job_id, instance_uuid, **kwargs):
        self.update_job_state_desc(job_id, _(u"开始停止虚拟机"))
        nova.get_client().servers.stop(instance_uuid)
        ...
        def checker():
            ...
            elif instance_ref['vm_state'] == vm_states.STOPPED:
                # 状态为 active, 停止完成
                return True
            return False
        ...
        # 等待完成或错误(包括超时)
        monitor.wait()
        self.update_job_state_desc(job_id, _(u"停止虚拟机成功"))

    def revert(self, job_id, *args, **kwargs):
        self.log_current_task_failures(*args, **kwargs)
        self.update_job_state_desc(job_id, _(u"停止虚拟机失败"))


def get_flow():
    flow = linear_flow.Flow(ACTION)
    flow.add(ShutdownOSInstanceTask())
    return flow


class InstanceShutdownBuilder(InstanceURDBuilder):
    def __init__(self, context, resource_kwargs=None, store=None):
        super(InstanceShutdownBuilder, self).__init__(
            context,
